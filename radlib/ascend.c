	...
	int require;
{
	...
	if (!_moreinput(pb)) {
		if (require) {
			asprintf(pb->errmsg, "Unexpected end of string");
			return NULL;
		}
		return NULL;
	}
	...
	struct ascend_parse_buf *pb;
{
	...
	if (strcmp(tok, "ip") == 0)
		...
		asprintf(pb->errmsg, "Unknown filter type: %s", tok);
		return 1;
	}
	return 0;
}
	...
	struct ascend_parse_buf *pb;
{
	...
	if (strcmp(tok, "in") == 0)
		...
		asprintf(pb->errmsg, "Unknown direction");
		return 1;
	}
	return 0;
}
	...
	struct ascend_parse_buf *pb;
{
	...
	if (strcmp(tok, "forward") == 0)
		...
		asprintf(pb->errmsg, "Unknown action");
		return 1;
	}
	return 0;
}
	...
	u_char *buf;
{
	...
	if (len > 2*ASCEND_MAX_CMP_LENGTH) {
		asprintf(pb->errmsg, "Octet string too long");
		return -1;
	}
	...
	for (p = tmp; len; len--, p++, tok++) {
		if (*tok >= 0 && *tok <= 9)
			*p = *tok - '0';
			...
			asprintf(pb->errmsg,
				 "Invalid hex character (near %s)", tok);
			return -1;
		}
	}
	...
	struct ascend_parse_buf *pb;
{
	...
	if (num != len) {
		asprintf(pb->errmsg,
			 "Value and mask are not of same size");
		return 1;
	}
	...
	if (strcmp(tok, "==") == 0)
		...
		asprintf(pb->errmsg, "Bad operator: %s", tok);
		return 1;
	}
	...
	if (strcmp(tok, "more") == 0)
		...
		asprintf(pb->errmsg, "Expected more but found %s",
			 tok);
		return 1;
	}
	return 0;
	...
	struct ascend_parse_buf *pb;
{
	...
	if (*p == 0) 
		...
		if (!p) {
			asprintf(pb->errmsg, "Unknown protocol %s", tok);
			return 1;
		}
	...
	int lookahead;
{
	...
	if (!lookahead)
		asprintf(pb->errmsg,
			 "expected {src|dst}port but found %s", tok);
	return ASCEND_DIR_NONE;
}
	...
	struct ascend_parse_buf *pb;
{
	...
	if (!tok)
		return ASCEND_DIR_NONE;
	ip = ipstr2long(tok); /*FIXME: no error checking */
	...
	struct ascend_parse_buf *pb;
{
	...
	if (_get_direction_type(pb, "ip", 1) != ASCEND_DIR_NONE) {
		int n1 = _get_ip(pb);
		if (n1 == n) {
			asprintf(pb->errmsg, "Duplicate IP specification");
			return 1;
		}
	}
	return 0;
	...
	struct ascend_parse_buf *pb;
{
	...
	asprintf(pb->errmsg, "Invalid operation: %s", s);
	return ascend_cmp_none;
}
	...
	struct ascend_parse_buf *pb;
{
	...
	if (*p == 0) 
		...
		if (!pp) {
			/* Shouldn't happen */
			asprintf(pb->errmsg,
				 "cannot map back the protol number");
			return ASCEND_DIR_NONE;
		}
		sp = getservbyname(tok, pp->p_name);
		if (!sp) {
			asprintf(pb->errmsg,
				 "Unknown service: %s", tok);
			return 1;
		}
		num = sp->s_port;
	}
	...
	char **errp;
{
	...
	*errp = NULL;
	if (argcv_get(input, "/", &pb.tokc, &pb.tokv)) {
		...
		asprintf(errp, "failed to tokenize");
		return 1;
	}
